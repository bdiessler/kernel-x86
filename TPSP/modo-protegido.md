...
Modo real:
16 bits
1 MiB de direccionamiento de memoria
no hay protección de memoria
desde este modo se pasa por software a modo protegido

Modo protegido:
32 bits
4 GiB de direccionamiento de memoria
si hay protección de memoria
compatibilidad con software existente
inaugurado por el 80286, primer procesador con multitasking
```
...
```
El modo real no tiene protección de memoria, por lo que es vulnerable a errores o
ataques. Además, el modo protegido tiene acceso a registros de 32 bits 
(o 64 si es compatible) y más direccionamiento de memoria, por protección 
tiene niveles de privilegio para restringir el acceso a memoria. Por último, 
se garantiza que los programas van a preservar compatibilidad en próximosprocesadores.
```
Anteriormente, detallamos que la memoria es un arreglo continuo de bytes y que 
podemos segmentarla de acuerdo a tamaño, nivel de protección y uso. Debemos indicar
al procesador la descripción de los segmentos, es decir, cómo están conformados los
segmentos. Los ejercicios a continuación tienen que ver con el armado de la tabla
de segmentos.
...
![](img/Segment%20Descriptor.png)
```
GDT: Global Descriptor Table 
Es una tabla de descriptores globales, ocupa 8 bytes. Esta contiene descriptores de segmento
especificando donde empieza, el tamaño, permisos de accesos y niveles de privilegio de cada
sección de memoria.
Base: Direccion fisica donde arranca la segmentación
Limit: Tamaño de la segmentación 
G: Granularidad, 0 → Bytes, 1 → 4KBytes
P: Indica si el segmento está en memoria, 0 →No esta, 1 → Esta
DPL: Mínimo nivel de privilegio necesario para acceder al segmento 
S: Describe el tipo de segmento, 0 → Segmento de sistema, 1 → Segmento de código o datos
```
...
```
La variable `extern gdt_entry_t gdt` representa un array de descriptores de segmentos.
La variable `extern gdt_descriptor_t GDT_DESC` representa al registro GDTR
```
...
```
La instrucción LGDT carga en GDTR la dirección base de nuestra GDT y su tamaño. En
el código es la GDT_DESC, un struct que almacena el tamaño de la gdt y un puntero a
su base y que se inicializa en `gdt.c`.
```
...
```
Usamos el selector del segmento de código de nivel 0. El offset es el inicio del
código protegido.
```
...
```
El tamaño es 50 * 80 * 2, que es la cantidad de píxeles con los dos bytes de
información que necesita cada uno.
```
...
```
La función escribe en pantalla un cuadrado formado por el char y el atributo de
entrada. Para acceder a memoria define un array en la posición base del buffer de
video y escribe ahí. Usa la estructura `ca`, que representa el caracter y el
atributo de color, ocupa 2 bytes.
```
