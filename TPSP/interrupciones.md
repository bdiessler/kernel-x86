...
offset: La dirección de entrada a la interrupción correspondiente. Cada interrupción va a
tener un offset diferente según la posición en el código.
segsel: El selector del segmento de código, guardado en la GDT, de nivel de kernel que
vamos a usar para ejecutar interrupciones.
type: El tipo de interrupción que va a represenatar esta entrada, si es de tareas, trap o
interrupción
dpl: El nivel de privilegio con el cual se puede llamar a esta interrupción.
present: si está presente el segmento, tiene que ser 1.
```
...
Deberíamos usar los valores 1110 (0xE) y usamos el selector de segmento `GDT_CODE_0_SEL`.
```
...
Las interrupciones de hardware deberían usar el `IDT_ENTRY0`.
```
...
El prólogo y el epílogo son los pushad y popad que guardan los registros de propósito
general de la tarea interrumpida, para que no se pierdan durante la interrupción. El iret
es el retorno de una interrupción a una tarea o procedimiento interrumpido, dá más
información que el ret, saca de la pila la información de la interrupción (el instruction
pointer, el selector de segmento de código y los EFLAGS) y hace un far return.
```
...
En principio no se puede, tendríamos que pasarle al reloj un número igual a la frecuencia
del clock del procesador y para eso necesitamos un número más grande que lo que los 16
bits nos permiten.
```
